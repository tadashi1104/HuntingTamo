...
class GameCommonFunction {
    ...
    //UIButtonの設定
    func setUIButton(text:String, textColor:UIColor, rect:CGRect, point:CGPoint, fontName:String, fontSize:CGFloat, target:AnyObject, action:Selector, tag:Int) -> UIButton {
        let button = UIButton()
        //表示されるテキスト
